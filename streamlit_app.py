...
def enviar_requisicao(input_text):
    if(input_text == ''):
        ...
        response = requests.get(f'http://mal-datadashboard.onrender.com/api/get_data/{input_text}')
...
if 'df' in st.session_state:
    ...
    if not df_filtered.empty:
        ...
        st.markdown("---")
        graph_nota_genero, graph_series_genero  = st.columns(2)
        st.markdown("---")
        graph_series_status, graph_avg_episodes_by_status  = st.columns(2)
        st.markdown("---")
        ...
        total_episodes_watched = df_filtered['num_episodes_watched'].sum()
        ...
        episodios_assistidos.metric("episódios assistidos", total_episodes_watched if not pd.isnull(average_score) else 'Sem dados')
        # endregion
        ...
        if len(df_group_episodes['series_title']) == 0:
        ...
        #region gráfico de nota por genero
        df_avg_score_by_genre = df_filtered.explode('genres')
        df_avg_score_by_genre = df_avg_score_by_genre.groupby('genres', observed=True).agg(count=('series_title', 'count'),
                                                                                        avg_score=('my_score', 'mean')
                                                                                        ).sort_values('count', ascending=False).head(10).reset_index()
        fig_nota_genero = px.bar(df_avg_score_by_genre, x='genres', y='avg_score', title='Média de Nota por Gênero')
        fig_nota_genero.update_layout(
            xaxis_title='Gênero',
            yaxis_title='Média de Nota',
            yaxis=dict(range=[0,10]),   
        )

        if df_avg_score_by_genre.empty:
            #somente um texto dizendo que não ha dados o suficiente no meio do gráfico
            fig_nota_genero.add_annotation(
                text='Não há dados suficientes',
                showarrow=False,
                font=dict(size=20)
            )
        graph_nota_genero.plotly_chart(fig_nota_genero, use_container_width=True)

        #endregion
        
        #region gráfico de média de episódios por status
        df_avg_episodes = df_filtered.copy()
        df_avg_episodes['episodes_to_use'] = df_avg_episodes.apply(lambda row: row['series_episodes'] if row['series_episodes'] != 0 else row['num_episodes_watched'], axis=1)
        df_avg_episodes = df_avg_episodes.groupby('my_status', observed=True)['episodes_to_use'].mean().reset_index()
        fig_avg_episodes_by_status = px.bar(df_avg_episodes, x='my_status', y='episodes_to_use', title='Média de Episódios por Status')
        fig_avg_episodes_by_status.update_layout(
            xaxis_title='Status',
            yaxis_title='Média de episodios',
            
        )

        if df_avg_episodes.empty:
            fig_avg_episodes_by_status.add_annotation(
                text='Não há dados suficientes',
                showarrow=False,
                font=dict(size=20)
            )
        graph_avg_episodes_by_status.plotly_chart(fig_avg_episodes_by_status, use_container_width=True)
        ...
        st.error('Nenhum dado encontrado, Tente alterar os filtros')